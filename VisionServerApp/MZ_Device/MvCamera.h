...
#include <sstream>
//缺陷包回调函数
typedef void (__stdcall* MSVIMAGEDATACALLBACK)(unsigned char* pData, MV_FRAME_OUT_INFO_EX* pFrameInfo, void* pUser);

class CMvCamera
{
    friend void __stdcall ReconnectDevice(unsigned int nMsgType, void* pUser);
public:
    CMvCamera(MV_CC_DEVICE_INFO* DevInfo);
    ~CMvCamera();
    ...
    static std::string GetDeviceIp(MV_CC_DEVICE_INFO* inputInfo);
    void SetMainThreadExit(bool input);

private:

    void*               m_hDevHandle;
    MSVIMAGEDATACALLBACK m_procallbackfun = NULL;
